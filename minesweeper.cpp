...
void reveal(int i, int j) {
    ...
    for (int di = -1; di <= 1; ++di) {
        for (int dj = -1; dj <= 1; ++dj) {
            int ni = i + di;
            int nj = j + dj;
            if (ni >= 0 && ni < height && nj >= 0 && nj < width && isMine(ni, nj))
                surrounding_mines++;
        }
    }
    setCell(i, j, surrounding_mines);
...
void executeCmd(char commandType, int rowPosition, int columnPosition) {
    bool cellHasMine = isMine(rowPosition, columnPosition);

    if (commandType == 'f') {
        if (cellHasMine) {
            setCell(rowPosition, columnPosition, FLAGGED_MINE);
            num_correct_flagged_mine += 1;
        } else {
            setCell(rowPosition, columnPosition, INCORRECT_FLAGGED_MINE);
    ...
    if (commandType == 'r') {
        if (cellHasMine) {
            ...
            reveal(rowPosition, columnPosition);
        }
    }

    if (commandType == 'u') {
        if (cellHasMine) {
            setCell(rowPosition, columnPosition, UNFLAGGED_MINE);
            num_correct_flagged_mine -= 1;
        } else {
            setCell(rowPosition, columnPosition, UNKNOWN);
            num_incorrect_flagged_mine -= 1;
        }
    }
}
    ...
    // After executing the command, check for game over or victory conditions if necessary
    // This could involve iterating over all cells to determine if the winning conditions are met
